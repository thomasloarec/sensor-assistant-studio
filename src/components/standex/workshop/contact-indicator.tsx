...
export default function ContactIndicator({ contact }: { contact: Contact }) {
  ...
  return (
    <section
      ...
      aria-label="Lecture électrique du contact"
    >
      <div className="mw-contact-copy" role="status" aria-live="polite">
        ...
        <div>
          <p>À L'INTÉRIEUR DU CAPTEUR</p>
          <h2>{closed ? "Contact fermé" : unknown ? "État indéterminé" : "Contact ouvert"}</h2>
          <span>
            {closed
      ...
      >
        ...
        <text x="163" y="63" textAnchor="middle">
          {closed ? "PASSAGE DU COURANT" : unknown ? "NON DÉTERMINÉ" : "CIRCUIT INTERROMPU"}
        </text>
      </svg>
      <p className="mw-circuit-note">
        <Lightbulb size={13} />
        Circuit alimenté fictif · aucun courant réel calculé
      </p>
    </section>
  );
}
